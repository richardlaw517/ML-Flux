...
%                 end
    ...
    textrep=[textrep; textrep2.(sheets{1}); textrep2.(sheets{2})];
    remove_cons=find(sum(ineq.A,2)==0);
    for i=1:length(remove_cons)
        if min(ineq.A(remove_cons(i),:))==0
            ineq.b(remove_cons(i))=0; % remove constraints on nonexistent reactions
        end
    end
end
